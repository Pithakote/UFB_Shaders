			...
			if (attached == false && moving == true && lerp_start + anim_time + air_time <= Time.time)
			{
				if (moving_to_screw == true)
				{
					...
					if (icon == null)
					{
						...
						icon.transform.position = transform.position + new Vector3(0,0.1f,0);
						Vector3 localAng = transform.eulerAngles;
						icon.transform.eulerAngles = new Vector3(90, 0, 90);
						//icon.transform.localScale = new Vector3(0.025f, 0.025f, 0.025f);

						if (newScale != null && newScale != 0)
							icon.transform.localScale = new Vector3(newScale, newScale, newScale);
